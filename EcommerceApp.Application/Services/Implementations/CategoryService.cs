...
{
    public class CategoryService(IGeneric<Category> categoryRepository, IMapper mapper,ICategory category) : ICategoryService
    {
        ...
        {
            var products = await category.GetProductsByCategory(categoryId);
            if (!products.Any())
                return [];
